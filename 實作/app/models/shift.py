...
class Shift(db.Model):
    ...
    def check_scheduling_conflict(cls, user_id, start_time, end_time, exclude_shift_id=None):
        ...
        # 1. 檢查不可上班偏好
        # weekday(): Monday is 0, Sunday is 6. 我們 preferences 中: 0=週日, 1=週一... 6=週六
        # 需要轉換 weekday():
        # datetime.weekday() 0(一), 1(二), 2(三), 3(四), 4(五), 5(六), 6(日)
        # 我們 preferences 週日為0 -> 轉換關係: (dt.weekday() + 1) % 7
        ...
        # 如果是更新班次，應扣除原班次工時，這裡暫不考慮或由 Controller 精細計算
    ...
    def get_shifts_by_range(cls, start_time, end_time, include_deleted=False, is_draft=None):
        ...
        if is_draft is not None:
            query = query.filter(cls.is_draft == is_draft)
        return query.all()
